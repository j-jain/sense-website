/* ════════════════════════════════════════════
   ...
   CSS sizes this panel). The truck no longer just ROTATES when it reroutes —
   it physically GLIDES along the detour path to Ahmedabad (sampled with
   getPointAtLength), turning onto the new road as it goes while the red route
   draws just ahead of it. reset() runs via onStart so the loop replays clean
   (the truck transform is set manually, outside GSAP's tween bookkeeping).
════════════════════════════════════════════ */
...
(function(){
  ...
  var tl = g.timeline({
    ...
    onUpdate:function(){
      ...
      if(timerEl){ var secs = Math.round(90 + this.progress()*72);
  ...
  if(det){
    ...
    tl.to(det,{strokeDashoffset:0,duration:0.16,ease:'power2.inOut'},0.54);
  }
  var glide={p:0};
