/* ════════════════════════════════════════════
   S5C — DRIVER & FLEET HANDOFF
   Lives on the sideways Scene-5 stage (no pin / no inline height — the stage
   CSS sizes this panel; mobile column handled there). Two deliberate beats:
     1) TEXT-FIRST — the right-side headline reveals fully and alone, then the
        phone + FleetMatters dashboard load in and the headline recedes.
     2) reset() via onStart so the loop replays clean — the scan→resolve cards
        mutate classes / inject DOM / rewrite text outside GSAP's bookkeeping.
════════════════════════════════════════════ */
...
(function(){
  ...
  /* Mouse-tracking parallax on the phone */
  ...
  /* Clean-loop reset for the non-GSAP mutations */
  function reset(){
  ...
  var s5ctl = g.timeline({
    ...
    onUpdate:function(){
      ...
      if(timerEl){ var secs=Math.round(162 + this.progress()*90);
  ...
  s5ctl.to('#s5c-headline',  {opacity:0.5, duration:0.1, ease:'power2.out'}, bUI+0.03);
